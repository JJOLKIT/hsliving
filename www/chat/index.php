...
if(!isset($_SESSION['session_fk'])){
	$rand = mt_rand(1, 9999);
	$_SESSION['rand'] = $rand;
	$_SESSION['session_fk'] = strtotime(Date('Y-m-d H:i:s')).$rand;
}

include "config.php";
$today = getToday();
$_REQUEST['scomdate'] = $today;
$_REQUEST['ssession_fk'] = $_SESSION['session_fk'];
...
$_REQUEST['offset'] = $rowPageCount[0]-5;


$req['sgb'] = 1;
$req['scomdate'] = $today;
$req['ssession_fk'] = $_SESSION['session_fk'];
$rowPageCount2 = $notice->getCount($req);
...
function getList(){
	$.ajax({
		...
		cache : false,
		data : {
			'scomdate' : '<?=$_REQUEST[scomdate]?>',
			'ssession_fk' : '<?=$_REQUEST[ssession_fk]?>',
			'cmd' : 'list'
...
function sendMsg(){
	...
	$.ajax({
		...
		async : false,
		data : {
			'comdate' : '<?=$_REQUEST[scomdate]?>',
			'session_fk' : '<?=$_REQUEST[ssession_fk]?>',
			'cmd' : 'write',
			'msg' : msg,
			'name' : name
		},
		success : function(data){
			...
			console.log(r);
			if(r == "success"){
				var c = "<li class='clear request'><p><?=$today?></p><span>"+name+"</span><div class='msg'>"+msg+"</div></li>";
				$('.chatlist').append(c);
...
form #btn {display:inline-block; vertical-align:middle; height:35px; padding:0 5px; box-sizing:border-box; background:#333; border:1px solid #333;  width:50px; cursor:pointer; color:#fff; }
	...
	.bbs .chatlist {min-height:600px; border:1px solid #aaa ;box-sizing:border-box; width:100%; position:relative; padding:20px; background:rgba(178,199,217,.8); overflow:auto; max-height:600px;}
	.bbs .chatlist dl {min-height:25px;  margin-top:20px;  width: 100%; line-height:25px;}
	.bbs .chatlist dl:first-child {margin-top:0;}
	.clear:after {clear:both; content:''; display:block;}
	.bbs .chatlist dl dt {display:inline-block;  font-weight:bold;}
	.bbs .chatlist dl .msg { width:auto; display:inline-block; padding:5px; border-radius:5px; position:relative;}
	.bbs .chatlist .response {float:left;}
	.bbs .chatlist .request {float:right; text-align:right;}
...
<div class="program">
	...
	<h2 class="title">관리자와 채팅</h2>
	
	<div class="bbs">
		<div id="list" class="chatlist">
		<?if($rowPageCount[0] > 0){?>
		<?while($row=mysql_fetch_assoc($result)){?>
		<dl <?if($row['gb'] == 1){ echo "class='response'";}else{ echo "class='request'";}?>>
			<dt><?=$row['name']?></dt>
		...
		<form onsubmit="chatManager.write(this); return false;">
			<input name="name" id="name" type="text" value="사용자<?=$_SESSION['rand']?>" readonly/>
