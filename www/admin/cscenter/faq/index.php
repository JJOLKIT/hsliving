...
<style>
	.searchWrap .searchTable{min-width:700px;}
</style>
</head>
	...
	<div class="contWrap">
				...
				<table class="searchTable">
					...
					<tbody>
						<tr>
							...
							<?if($category_use){?>
								<span class="select">
									<select name="scategory_fk" id="scategory_fk" onchange="goSearch();">
										<option value="">분류 선택</option>
										<? while ($row=mysql_fetch_assoc($category_result)) { ?>
										<option value="<?=$row[no]?>" <?=getSelected($row[no], $_REQUEST['scategory_fk']) ?>><?=$row[name]?></option>
										<? } ?>
									</select>
								</span>
							<?}?>
		...
		<div class="list">
			...
			<table>
				...
				<? if ($rowPageCount[0] == 0) { ?>
					<tr>
						<td colspan="<?=$category_use == true ? "5" : "4"?>" align="center">등록된 글이 없습니다.</td>
					</tr>
				<?
					 } else {
						$targetUrl = "";
				...
				?>
					<tr <?=$topClass?>>
						...
						<? } else { ?>
							<?=$rowPageCount[0] - (($faq->reqPageNo-1)*$pageRows) - $i?>
						<? } ?>
						</td>
						<?if($category_use){?>
						<td <?=$targetUrl ?>><?=$row[category_name]?></td>
						<?}?>
						<td class="txt_l" <?=$targetUrl ?>><?=$row[title]?>
			...
			<?=$faq->getQueryStringToHidden($_REQUEST) ?>
			</form>
		</div>
		<!-- //list -->
