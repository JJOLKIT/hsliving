...
if (checkReferer($_SERVER["HTTP_REFERER"])) {
	...
	if ($_REQUEST['cmd'] == 'write') {
		...
		if ($r > 0) {
			echo returnURLMsg($notice->getQueryString(getRemoviSslUrl($_SERVER["REQUEST_URI"], 'view.php'), $_REQUEST['no'], $_POST), '정상적으로 수정되었습니다.');
		} else {
			echo returnURLMsg($notice->getQueryString(getRemoviSslUrl($_SERVER["REQUEST_URI"], 'edit.php'), $_REQUEST['no'], $_POST), '요청처리중 장애가 발생하였습니다.');
		}
		}
		...
		if($r > 0){
			...
			echo returnURLMsg($notice->getQueryString(getRemoviSslUrl($_SERVER["REQUEST_URI"], 'edit.php'), $_REQUEST['no'], $_REQUEST), '요청처리중 장애가 발생하였습니다.');	
		}
