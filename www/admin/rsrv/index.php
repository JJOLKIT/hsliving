...
<!-- s:warp -->
	<div class="contWrap"> 
						...
						<td>
							...
							<? 
									...
									for($v = 0; $v < count($list); $v++){
										...
										if( $list[$v]['category'] == 1 ) {
											?>
											<a href="<?=$notice->getQueryString('view.php', $list[$v][no], $_REQUEST)?>" class="tcname tc1 clear state<?=$list[$v]['state']?>"><span class="fl_l"><?=getPlaceName($list[$v]['place'])?></span><span class="fl_r"><?=substr($list[$v]['rtime'], 0, 5)?> ~ <?=$lastTime.":00"?></span></a>
											<?
										}else if($list[$v]['category'] == 2) {
											?>
											<a href="" class="tcname tc2 clear"><span class="fl_r"><?=substr($list[$v]['rtime'], 0, 5)?> ~ <?=$lastTime.":00"?></span></a>
											<?
										}
									}
		...
		<div class="caption mt20">
			<ul>
				<li>
					<span style="color:green; background:#ddd; padding:1px 5px; display:inline-block; opacity:0.8;">접수</span> <span style="opacity:1; background:#ffd247; color:black; padding:1px 5px; display:inline-block;">확정</span>
					<span style="color:red; background:#ddd; padding:1px 5px; display:inline-block; opacity:0.8;">취소신청</span> <span style="opacity:0.8; background:#ddd; color:gray; padding:1px 5px; display:inline-block;">취소확정</option>
				</li>
