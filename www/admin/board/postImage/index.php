...
<div id="wrapper">
	<div class="contWrap">
		...
		<!-- //search_warp -->
		<div class="list">
			<p class="list_tit clear">전체 <strong><?=$rowPageCount[0]?></strong>건 [<?=$post->reqPageNo?>/<?=$rowPageCount[1]?>페이지] 
				<span>
			...
			<table>
				...
				<?
						...
						while ($row=mysql_fetch_assoc($result)) {
							$targetUrl = "style='cursor:pointer;' onclick=\"location.href='".$post->getQueryString('view.php', $row[no], $_REQUEST)."'\"";
							if ($row[top] == '1') {
				...
				?>
					<tr <?=$topClass?>>
						...
						<? } else { ?>
							<?=$rowPageCount[0] - (($post->reqPageNo-1)*$pageRows) - $i?>
						<? } ?>
						</td>
						<td <?=$targetUrl?>>
						<? if ($row['imagename']) { ?>
							<img src="<?=$uploadPath?><?=$row['imagename']?>" class="listImg"/>
						<? } ?>
						</td>
			...
			<?=$post->getQueryStringToHidden($_REQUEST) ?>
		</form>
		</div>
		<!-- //list -->
