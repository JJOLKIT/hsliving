...
$(window).load(function() {
	...
	function changeClass(){
		$.get("selectClassList.php?name=class_fk&syear_type="+$("#year_fk").val(), function (data, status) {
			$("#classDiv").html(data);
		});
		//goSearch();
	}
			...
			<div class="wr_box">
				...
				<table>
					...
					<tr>
						...
						<td>
							<? if ($data['moviename']) { ?>
								<input type="checkbox" name="moviename_chk" value="1"/> 기존파일삭제</br/>
								<a href="/lib/download.php?path=<?=$uploadPath?>&vf=<?=$data['moviename']?>&af=<?=$data['moviename_org']?>" target="_blank"><?=$data[moviename_org]?></a><br/>
							<? } ?>
