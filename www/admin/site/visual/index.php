...
function changeSeq(type, no, seq){
	$.ajax({
		url : 'seq.php',
		data : {
			'type' : type,
			...
			<table>
				...
				<thead>
					<tr>
						...
						<th scope="col">제목</th>
						<th scope="col">PC 이미지</th>
						<th scope="col">모바일 이미지</th>
				...
				?>
					<tr <?=$topClass?>>
						...
						<td <?=$targetUrl ?>><?=$rowPageCount[0] - (($notice->reqPageNo-1)*$pageRows) - $i?></td>
						<td <?=$targetUrl ?>>
							<?=getDisplay($row['main']);?>
						</td>
						<td class="txt_l" <?=$targetUrl ?>><?=$row[title]?></td>
						<td <?=$targetUrl ?>>
							<? if ($row[imagename]) { ?>
							<img src="<?=$uploadPath?><?=$row[imagename]?>" style="max-height:120px;"/>
							<? } ?>
						</td>
						<td <?=$targetUrl ?>>
							<? if ($row[filename]) { ?>
							<img src="<?=$uploadPath?><?=$row[filename]?>" style="max-height:120px;"/>
							<? } ?>
						</td>
						<td class="seq">
							<input type="text" value="<?=$row[seq]?>" disabled class="txt_c" class="wid100"/><br/>
							<?if($i != 0){?>
							<input type="button" value="위로" onclick="changeSeq('up', '<?=$row['no']?>', '<?=$row['seq']?>');"/>
							<?}?>

							<?if(($i+1) != $rowPageCount[0] ){?>
							<input type="button" value="아래로" onclick="changeSeq('down', '<?=$row['no']?>', '<?=$row['seq']?>');"/>
							<?}?>
						</td>
