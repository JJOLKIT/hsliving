...
if (checkReferer($_SERVER["HTTP_REFERER"])) {
	...
	} else if ($_REQUEST['cmd'] == 'copy') {
		$_POST = xss_clean($_POST);


		if($_FILES['imagename_new']['name'] != ""){

			$_POST = fileupload('imagename_new', $_SERVER['DOCUMENT_ROOT'].$uploadPath, $_POST, false, $maxSaveSize);	// 이미지
			$_POST['imagename'] = $_POST['imagename_new'];
			$_POST['imagename_org'] = $_POST['imagename_new_org'];
		}
