	...
	<div class="contWrap">
							...
							<td>
								<span class="select">
									<select name="scategory" id="scategory" onchange="goSearch();">
										...
										<option value="<?=$row['no']?>" <?=getSelected($row['no'], $_REQUEST['scategory'])?>><?=$row['title']?></option>
										<?}?>
									</select>
								</span>
							</td>
		...
		<div class="list">
			...
			<? } else { ?>
				...
				?>
					<li>
						<p class="chk clear"><input type="checkbox" name="no[]" id="no" value="<?=$row[no]?>"/> <?=$rowPageCount[0] - (($notice->reqPageNo-1)*$pageRows) - $i?>
						...
						<dl>
							...
							<dd class="category color">[<?=$row['category_title']?>]</dd>
							<dd class="title">
								<?=mb_strimwidth($row['title'], '0', '80', '...','utf-8')?>
							</dd>
							<dd class="info">
								<ul>
									<li>진행 기간 : <?=($row['stday'])?><!-- ~ <?=$row['etday']?>--></li>
									<li>모집 기간 : <?=($row['sday'])?> ~ <?=$row['eday']?></li>
