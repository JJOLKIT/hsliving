...
<!-- s:warp -->
	<div class="contWrap">
		...
		<div class="write">
			...
			<div class="wr_box">
				...
				<table>
					...
					<tr>
						...
						<td>
							<? if ($data['filename2']) { ?>
								<a href="/lib/download.php?path=<?=$uploadPath?>&vf=<?=$data['filename2']?>&af=<?=urlencode($data['filename2_org'])?>" target="_blank"><?=$data[filename2_org]?></a>
							<? } ?>
						</td>
					</tr>
					<tr>
						...
						<td>
							<? if ($data['filename3']) { ?>
								<a href="/lib/download.php?path=<?=$uploadPath?>&vf=<?=$data['filename3']?>&af=<?=urlencode($data['filename3_org'])?>" target="_blank"><?=$data[filename3_org]?></a>
							<? } ?>
						</td>
					</tr>
					</tbody>
			...
			<!-- //wr_box -->
			<? if ($isComment) { ?>
				<? include $_SERVER['DOCUMENT_ROOT']."/admin/cs/comment/comment.php" ?><!-- 댓글 -->
			<? } ?>
		</div>
		<!-- //write -->
		<div class="btnSet">
			<span class="left">
				<a href="<?=$notice->getQueryString('index.php', 0, $_REQUEST) ?>" class="btn hoverbg list">
