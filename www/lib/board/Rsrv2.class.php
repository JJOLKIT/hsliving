...
class Rsrv2 {
	...
	// 생성자
	function Rsrv2($pageRows=0, $tableName='', $request='') {
	...
	function getWhereSql($p) {
		...
		if ($p['sval']) {
			if ($p['stype'] == 'all') {
				$whereSql .= " AND ( (name like '%".$p['sval']."%' ) or (cell like '%".$p['sval']."%') OR (program_fk IN (SELECT no FROM program WHERE title like '%".$p['sval']."%')) ) ";
			} else if($p['stype'] == "program_title"){
				$whereSql .= " AND (
					program_fk IN (SELECT no FROM program WHERE title like '%".$p['sval']."%')
				)";
			}else {
		...
		if ($p['sstartdate'] != '') {
			...
			if ($p['senddate'] != '') {
				$whereSql .= " AND (rdate BETWEEN '".$p['sstartdate']."' AND '".$p['senddate']."') ";
			}
		}
		...
		if($p['sprogram_fk'] != ""){
			$whereSql .= " AND program_fk = ".$p['sprogram_fk'];
		}
		...
		/*
		if($p['srdate'] != ""){
			$whereSql .= " AND rdate = '$p[srdate]' ";
		}
		*/

		if($p['srdate'] != ""){
			$whereSql .= " AND (program_fk IN (SELECT no FROM program WHERE stday = '".$p['srdate']."') )";
		}
...
// 		}
	...
	function getList($param='') {
		...
		$sql = "
			SELECT *,
			(SELECT title FROM program WHERE no = ".$this->tableName.".program_fk) AS title
			FROM ".$this->tableName."
			".$whereSql."
	...
	function insert($req="") {
		...
		$sql = "
			...
			member_fk, program_fk, rdate, rtime, gender, together, birthday, addr,  dc, dc_txt, price, state, pay_state, cell, name, email,relation_url, registdate, ";
		if ($req[filename]) {
	...
	function update($req="") {
		...
		$sql = "
			...
			state = ".chkIsset($req[state]).", gender = ".chkIsset($req[gender]).", together = ".chkIsset($req[together]).",
				birthday = '$req[birthday]',
				cell = '$req[cell]', addr = '$req[addr]',
				dc = ".chkIsset($req[dc]).", dc_txt = ".chkIsset($req[dc_txt]).",
	...
	function userUpdate($req="") {
		...
		$sql = "
			UPDATE ".$this->tableName." SET 
			member_fk = ".chkIsset($req[member_fk]).", program_fk = ".chkIsset($req[program_fk]).", rdate = '$req[rdate]', rtime = '$req[rtime]', gender = ".chkIsset($req[gender]).", together = ".chkIsset($req[together]).", birthday = '$req[birthday]', addr = '$req[addr]', dc = ".chkIsset($req[dc]).", dc_txt = ".chkIsset($req[dc_txt]).", price = ".chkIsset($req[price]).", state = ".chkIsset($req[state]).", pay_state = ".chkIsset($req[pay_state]).", cell = '$req[cell]', 
	...
	function getData($no=0, $userCon) {
		...
		$sql = "
			SELECT *,
			(SELECT title FROM program WHERE no = ".$this->tableName.".program_fk) AS title,
			(SELECT stday FROM program WHERE no = ".$this->tableName.".program_fk) AS stday
			FROM ".$this->tableName."
			WHERE no = ".$no;
	...
	function getDetailList($req=''){
		...
		//	(SELECT COUNT(*) FROM rsrv2 WHERE state < 3 AND rdate = '$req[srdate]') AS cnt,
	//		(SELECT SUM(together) FROM rsrv2 WHERE state < 3 AND rdate = '$req[srdate]') as together,
	$sql .= "
	(SELECT COUNT(*) FROM rsrv2 WHERE (state < 3 OR state = 5) AND program_fk IN ( SELECT no FROM program WHERE stday = '$req[srdate]') ) AS cnt,
	(SELECT SUM(together) FROM rsrv2 WHERE (state < 3 OR state = 5) AND program_fk IN ( SELECT no FROM program WHERE stday = '$req[srdate]') ) as together,
			rtime, 
			(SELECT title FROM program WHERE no = rsrv2.program_fk) as title,
			(SELECT amount FROM program WHERE no = rsrv2.program_fk) AS amount
			 FROM rsrv2 WHERE ";
			// rdate= '$req[srdate]' 

			$sql .= "
			program_fk IN (SELECT no FROM program WHERE stday = '$req[srdate]')
			AND (SELECT no FROM program WHERE no = rsrv2.program_fk) IS NOT NULL
			 GROUP BY program_fk
		";
		outlog($sql);
		$result = mysql_query($sql, $conn);
	...
	function getProgramList($req=''){
		$dbconn = new DBConnection();
		$conn = $dbconn->getConnection();

		$sql = "
			SELECT *,
			(SELECT COUNT(*) FROM rsrv2 WHERE program_fk = program.no AND (state < 3 OR state = 5) ) AS cnt,
			(SELECT SUM(together) FROM rsrv2 WHERE program_fk = program.no AND (state < 3 OR state = 5) ) AS together
			FROM program WHERE stday = '$req[srdate]' AND display = 1 
		";
