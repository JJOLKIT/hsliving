...
class Chat {
	...
	// 생성자
	function Chat($pageRows=0, $tableName='', $request='') {
	...
	function getWhereSql($p) {
		...
		if($p['sstartdate'] != ""){
			$whereSql .= " AND registdate > '".$p[sstartdate]."'";
		}
// 		if ($p['smain'] != '') {
...
// 		}
		...
		if($p['ssession_fk'] != ""){
			$whereSql .= " AND session_fk =".chkIsset($p['ssession_fk']);
		}
		if($p['scomdate'] != ""){
			$whereSql .= " AND comdate = '".$p['scomdate']."'";
		}
		if($p['sgb'] != ""){
	...
	// 전체로우수, 페이지카운트
	function getAdminCount($param = "") {
		$dbconn = new DBConnection();
		$conn = $dbconn->getConnection();
		
		$whereSql = $this->getWhereSql($param);	// where절
		$sql = " SELECT COUNT(c.session_fk) AS cnt FROM (SELECT session_fk, count(session_fk) FROM chat WHERE 1=1 GROUP BY session_fk) as c";
	...
	function insert($req="") {
		...
		$sql = "
			INSERT INTO ".$this->tableName." (
				session_fk, msg, name, comdate,  registdate, ";
		if ($req[filename]) {
...
outlog($sql);
	...
	function updateState($req){
		...
		$sql = "
			UPDATE ".$this->tableName." SET state = ".chkIsset($req['state'])." WHERE session_fk = ".$req['session_fk'];
	...
	function getNewCount($req){
		$dbconn = new DBConnection();
		$conn = $dbconn->getConnection();
		
		$sql = "SELECT COUNT(*) AS cnt FROM chat WHERE state=0 AND comdate = '".$req['scomdate']."' AND gb = 0";
		$result = mysql_query($sql, $conn);
