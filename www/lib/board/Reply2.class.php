...
class Reply2 {
	...
	// 생성자
	function Reply2($pageRows=0, $tableName='', $request='') {
	...
	function insert($req="") {
		...
		$sql = "
			INSERT INTO ".$this->tableName." (
				member_fk, gno, ono, nested, password, name, cell, email, title, contents, relation_url, registdate, ";
		if ($req[filename]) {
		...
		$sql .= "top, main, newicon, secret
				...
				".chkIsset($req[member_fk]).", $gno, 0, 0, ".DB_ENCRYPTION."('$req[password]'), '$req[name]', '$req[cell]', '$req[email]', '$req[title]', '$req[contents]', '$req[relation_url]',";
...
outlog($sql);
	...
	function answer($req="") {
		...
		$sql .= "top, main, newicon, secret
				...
				".chkIsset($req[member_fk]).", $req[gno], $ono, $nested, '$req[password]', '$req[name]', '$req[cell]', '$req[email]', '$req[title]', '$req[contents]', '$req[relation_url]', ";
		if ($req[registdate]) {	
			$sql .= " '$req[registdate]', ";
		} else {
			$sql .= " NOW(), ";
	...
	function update($req="") {
		...
		$sql = "
			...
			an_title='$req[an_title]', an_contents='$req[an_contents]', state = 1,
			";
		if ($req[andate]) {
			$sql .= " an_date = NOW(), ";
		}
	...
	function userUpdate($req="") {
		...
		$sql = "
			UPDATE ".$this->tableName." SET 
			member_fk=".chkIsset($req[member_fk]).", name='$req[name]', cell='$req[cell]', email='$req[email]', title='$req[title]', contents='$req[contents]', relation_url='$req[relation_url]',
			";
