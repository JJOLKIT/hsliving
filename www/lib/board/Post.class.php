...
class Post {
	...
	// 생성자
	function Post($pageRows=0, $tableName='', $request='') {
	...
	function insert($req="") {
		...
		$sql = "
			INSERT INTO ".$this->tableName." (
				name, email, title, post_option, contents, relation_url, registdate, ";
		if ($req[filename]) {
	...
	function update($req="") {
		...
		$sql = "
			UPDATE ".$this->tableName." SET 
				name='$req[name]', email='$req[email]', title='$req[title]', post_option ='$req[post_option]', contents='$req[contents]',
