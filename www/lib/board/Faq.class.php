...
class Faq {
	...
	var $tableName;			// 테이블명
	var $category_tablename;	// 카테고리 테이블명
	...
	// 생성자
	function Faq($pageRows=0, $tableName='', $category_tablename='', $request='') {
	...
	function getList($param='') {
		...
		$sql = "
			SELECT *";
		if ($this->category_tablename) { 
			$sql .= ", (SELECT name FROM ".$this->category_tablename." WHERE ".$this->category_tablename.".no = ".$this->tableName.".category_fk) AS category_name ";
		}
	...
	function insert($req="") {
		...
		$sql = "
			INSERT INTO ".$this->tableName." (
				category_fk, title, contents, relation_url, registdate, ";
		if ($req[filename]) {
	...
	function update($req="") {
		...
		$sql = "
			UPDATE ".$this->tableName." SET 
				category_fk=".chkIsset($req[category_fk]).", title='$req[title]', contents='$req[contents]',
