...
class Fb {
	...
	// 생성자
	function Fb($pageRows=0, $tableName='', $request='') {
	...
	function getWhereSql($p) {
		...
		if ($p['sval']) {
			...
			} else if($p['stype'] == "name"){
				$whereSql .= " AND (name = '".$p['sval']."') ";
			}else {
		...
		if($p['sadmin_fk'] != ""){
			$whereSql .= " AND admin_fk = ".chkIsset($p['sadmin_fk']);
		}
		if($p['sgubun'] != ""){
			$whereSql .= " AND gubun = '".$p['sgubun']."'";
		}
...
// 		}
	...
	function getList($param='') {
		...
		$sql = "
			...
			FROM ".$this->tableName."
			".$whereSql."
			ORDER BY no asc LIMIT ".$this->startPageNo.", ".$this->pageRows." ";
	...
	function insert($req="") {
		...
		$sql = "
			INSERT INTO ".$this->tableName." (
				name, relation_url , admin_fk, gubun
			";
			if($req['tablename']){
				$sql .= ", tablename";
			}
	...
	function deleteByName($name='', $admin_fk=0){
		$dbconn = new DBConnection();
		$conn = $dbconn->getConnection();

		$sql = "
			DELETE FROM ".$this->tableName." WHERE name = '$name' AND admin_fk = ".chkIsset($admin_fk)."
		";
	...
	function getDashList ($req=''){
		$dbconn = new DBConnection();
		$conn = $dbconn->getConnection();
		$whereSql = $this->getWhereSql($req);

		$sql = "
			SELECT * FROM ".$this->tableName." ".$whereSql." ORDER BY no DESC LIMIT 0, 4";
		outlog($sql);
	...
	function getDashBoardList($req=''){
		$dbconn = new DBConnection();
		$conn = $dbconn->getConnection();

		$sql = "SELECT * FROM ".$req['tablename']." ORDER BY registdate DESC, no DESC LIMIT 0, 4";
	...
	function getDashBoardCount($req=''){
		$dbconn = new DBConnection();
		$conn = $dbconn->getConnection();

		$sql = "SELECT COUNT(*) AS cnt FROM ".$req['tablename']."";
