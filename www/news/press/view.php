				...
				<div class="bbs con_info">
					<div class="view">
						...
						<div>
							<ul class="viewNavi">
								<?if($next){?>
								<li class="prev"><a href="<?=$notice->getQueryString('view.php', $next[next_no], $_REQUEST)?>"><?=$next['next_title']?></a></li>
								<?}?>
								<?if($prev){?>
								<li class="next"></span><a href="<?=$notice->getQueryString('view.php', $prev[prev_no], $_REQUEST)?>"><?=$prev['prev_title']?></a></li>
								<?}?>
							</ul>
						</div>
						<?}?>
